...
/// </summary>
public partial class CommandSetSettingsPage
{
    private readonly ObservableCollection<CommandSet> _commandSets;
    private readonly ObservableCollection<CommandConfig> _currentCommands;

    public CommandSetSettingsPage()
    {
        InitializeComponent();
        // Initialize data collections
        _commandSets = new ObservableCollection<CommandSet>();
    ...
    private void LoadCommandSets()
    {
        ...
        {
            _commandSets.Clear();
            var commandsDirectory = PathManager.GetCommandsDirectoryPath();
            var registryFilePath = PathManager.GetCommandRegistryFilePath();
            // 1. First load all command set folders, establish available command collections
            var availableCommandSets = new Dictionary<string, CommandSet>();
            var availableCommandNames = new HashSet<string>();
            // Get all command set directories
            var commandSetDirectories = Directory.GetDirectories(commandsDirectory);
            foreach (var directory in commandSetDirectories)
            {
                ...
                if (Path.GetFileName(directory).StartsWith("."))
                    continue;
                var commandJsonPath = Path.Combine(directory, "command.json");
                // If there's a command.json, load it
                if (File.Exists(commandJsonPath))
                {
                    var commandJson = File.ReadAllText(commandJsonPath);
                    ...
                    if (commandSetData != null)
                    {
                        ...
                        foreach (var command in commandSetData.Commands)
                        {
                            ...
                            var supportedCommandVersions = new List<string>();
                            ...
                            foreach (var version in versionDirectories)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (dllFiles.Length > 0)
                                    {
                                        ...
                                        {
                                            // 提取DLL文件名
                                            var dllFileName = Path.GetFileName(versionDllPath);
            ...
            {
                var registryJson = File.ReadAllText(registryFilePath);
                ...
                if (registry?.Commands != null)
                {
                    ...
                    if (validCommands.Count != registry.Commands.Count)
                    {
                        ...
                        var updatedJson = JsonConvert.SerializeObject(registry, Formatting.Indented);
    ...
    private void RefreshButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        LoadCommandSets();
        // Restore selection
        if (selectedIndex >= 0 && selectedIndex < _commandSets.Count)
        {
    ...
    private void SaveButton_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            var registryFilePath = PathManager.GetCommandRegistryFilePath();
            // 读取现有的注册表以保留完整的命令信息
            var existingCommandsDict = new Dictionary<string, CommandConfig>();
            if (File.Exists(registryFilePath))
            {
                var registryJson = File.ReadAllText(registryFilePath);
            ...
            // 创建新的registry对象
            var registry = new CommandRegistryJson();
            registry.Commands = new List<CommandConfig>();
            // 收集所有"已启用"的命令
            foreach (var commandSet in _commandSets)
            {
                // 尝试从command.json中获取开发者信息
                var commandSetDeveloper = new DeveloperInfo { Name = "Unspecified", Email = "Unspecified" };
                var commandJsonPath = Path.Combine(PathManager.GetCommandsDirectoryPath(),
            ...
            var enabledCount = registry.Commands.Count;
            foreach (var command in registry.Commands)
            {
                var commandSetName = _commandSets
                    .FirstOrDefault(cs => cs.Commands.Any(c => c.CommandName == command.CommandName))?.Name ?? "Unknown";
                var versions = command.SupportedRevitVersions != null && command.SupportedRevitVersions.Any()
